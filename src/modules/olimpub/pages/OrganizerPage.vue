<template>
  <q-page class="op-scope op-organizer relative min-h-full overflow-x-hidden">
    ...
    <div class="op-inner">
      ...
      <section class="manage-panel">
        ...
        <div class="manage-panel__brand">
          <img :src="olimpubLogo" alt="Olimpub" />
        </div>
        <p class="manage-panel__tagline">{{ olimpubTagline }}</p>
      ...
      <div class="manage-grid">
        <button
          ...
          type="button"
          class="manage-tile"
          :class="{ 'op-play-tile': action.highlight }"
...
const manageActions = computed(() => {
  const items: Array<{
    ...
    { id: 'quiz', label: 'Quiz', icon: 'sym_r_quiz', highlight: true, onClick: openQuiz },
    { id: 'quizmaster', label: 'Kvízmester', icon: 'sym_r_sports_esports', highlight: true, onClick: openQuizmaster },
    { id: 'results', label: 'Eredmények', icon: 'sym_r_emoji_events', onClick: openResults },
    { id: 'display', label: 'Kivetítő', icon: 'sym_r_tv', onClick: openDisplay },
...
function openParticipants() {
  router.push({
    path: `/olimpub/event/${eventId.value}/participants`,
    query: route.query,
  });
}
...
function openQuiz() {
  router.push({
    path: `/olimpub/event/${eventId.value}/quiz`,
    query: route.query,
  });
}
...
function openDisplay() {
  const url = router.resolve({ path: `/olimpub/event/${eventId.value}/display` }).href;
  window.open(url, '_blank', 'noopener');
}

function openScan() {
  router.push({
    path: `/olimpub/event/${eventId.value}/scan`,
    query: route.query,
  });
}
...
function openMaterials() {
  router.push({
    path: `/olimpub/event/${eventId.value}/materials`,
    query: route.query,
  });
}
...
.op-play-tile {
  ...
  background:
    radial-gradient(circle at 50% 0%, rgba(245, 185, 66, 0.24), transparent 70%),
    rgba(12, 16, 28, 0.92);
  box-shadow: 0 0 0 1px rgba(245, 185, 66, 0.22), 0 8px 20px rgba(245, 185, 66, 0.16);
}
...
.op-play-tile .manage-tile__icon {
