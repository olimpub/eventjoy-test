<template>
  ...
  >
    ...
    <div class="relative h-[120px] w-full group overflow-hidden">
      ...
      <!-- Status Badge (Replacing the previous badges) -->
      <div class="absolute bottom-2 left-6 right-6 flex items-center gap-2 z-10">
        <span class="px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest shadow-lg flex items-center gap-1.5 transition-colors bg-brand-yellow text-brand-dark">
    ...
    <div class="px-6 mt-4 relative z-10">
      <!-- Title Header & Info Row -->
      <div class="mb-4">
        <h1 class="text-4xl font-black leading-[1.1] text-white tracking-tighter mb-2">{{ event.title }}</h1>
        
        <!-- Integrated Quick Info Row -->
        <div class="flex items-center gap-4 text-xs font-bold text-white/80">
      ...
      <section class="mt-4">
        <h2 class="text-xl font-bold mb-4 flex items-center gap-2">
          <div class="w-1 h-6 rounded-full" :style="{ backgroundColor: event.theme.primary }"></div>
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="flex flex-col gap-1.5">
              <label class="text-[10px] font-black uppercase tracking-widest text-white/50 ml-1">Vezetéknév</label>
              <input
                type="text"
                placeholder="Pl. Kovács"
                class="bg-white/5 border border-white/10 rounded-2xl px-5 py-3 text-sm font-bold focus:outline-none focus:border-[var(--event-primary)] transition-colors placeholder:text-white/20"
              />
            </div>
            <div class="flex flex-col gap-1.5">
              <label class="text-[10px] font-black uppercase tracking-widest text-white/50 ml-1">Keresztnév</label>
              <input
                type="text"
          ...
          <!-- Institution -->
          <div class="flex flex-col gap-1.5">
            <label class="text-[10px] font-black uppercase tracking-widest text-white/50 ml-1">Intézmény</label>
            <input
              type="text"
              placeholder="Iskola vagy munkahely neve"
              class="bg-white/5 border border-white/10 rounded-2xl px-5 py-3 text-sm font-bold focus:outline-none focus:border-[var(--event-primary)] transition-colors placeholder:text-white/20"
            />
          </div>
          ...
          <!-- Email -->
          <div class="flex flex-col gap-1.5">
            <label class="text-[10px] font-black uppercase tracking-widest text-white/50 ml-1">Email cím</label>
            <input
              type="email"
          ...
          <!-- Phone -->
          <div class="flex flex-col gap-1.5">
            <label class="text-[10px] font-black uppercase tracking-widest text-white/50 ml-1">Telefonszám</label>
            <input
              type="tel"
      ...
      <div class="mt-6 mb-10">
        <button
          @click="router.push(`/event/${eventId}/confirmation`)"
          class="w-full py-4 rounded-[2rem] font-black uppercase tracking-widest text-sm shadow-xl active:scale-95 transition-all"
          :style="{
...
// Mocked database of events (same as in EventDetailsView)
