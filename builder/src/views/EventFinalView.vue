<template>
  ...
  >
    ...
    <div class="relative h-[120px] w-full group overflow-hidden">
      ...
      <!-- Status Badge (RED: Játék vége), Toggle & Profile -->
      <div class="absolute bottom-2 left-6 right-6 flex items-center justify-between gap-2 z-10">
        <span class="px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest shadow-lg flex items-center gap-1.5 transition-colors bg-red-500 text-white">
    ...
    <div class="px-6 mt-4 relative z-10">
      ...
      <div class="mb-4 text-center">
        <span class="text-brand-cyan text-sm font-black uppercase tracking-[0.2em] animate-pulse">Eredmények hamarosan!</span>
      </div>

      <!-- Summary Stats (With Spinners) -->
      <div class="grid grid-cols-2 gap-3 mb-6">
        ...
        <div class="bg-white/5 border border-white/10 rounded-[2rem] p-4 backdrop-blur-md flex items-center gap-4">
          <div class="w-12 h-12 flex items-center justify-center p-2 rounded-xl bg-white/5 border border-white/10">
            <img src="https://cdn.builder.io/api/v1/image/assets%2F63f0f74a9e2d44888ec29c2efcf85788%2F3a7ae3260b1b4b83959336402dbcae12?format=webp&width=800&height=1200" alt="Total Score" class="w-full h-full object-contain opacity-40" />
          </div>
          <div class="flex flex-col">
            <span class="text-[9px] font-black uppercase tracking-widest text-white/40">Pontszám</span>
            <div class="mt-1 w-4 h-4 border-2 border-t-brand-cyan border-white/10 rounded-full animate-spin"></div>
          </div>
        </div>
        ...
        <div class="bg-white/5 border border-white/10 rounded-[2rem] p-4 backdrop-blur-md flex items-center gap-4">
          <div class="w-12 h-12 flex items-center justify-center p-2 rounded-xl bg-white/5 border border-white/10">
            <img src="https://cdn.builder.io/api/v1/image/assets%2F63f0f74a9e2d44888ec29c2efcf85788%2F4911b503290e438cb556b1335b181afe?format=webp&width=800&height=1200" alt="Total Rank" class="w-full h-full object-contain opacity-40" />
          </div>
          <div class="flex flex-col">
      ...
      <section class="space-y-3">
        ...
        >
          ...
          <div class="absolute top-4 right-6 flex items-center gap-2">
            <!-- No Eredmény button for Round 3 -->
            <button
              v-if="round.status === 'lezárt' && round.number !== 3"
            ...
            <div class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-white/20 text-white/40 bg-white/5">
              lezárt
            </div>
          </div>
          ...
          <!-- Results Grid (Only for completed rounds 1 and 2) -->
          <div
            v-if="round.number !== 3"
          ...
          <!-- Round 3 Display (No details yet) -->
          <div v-if="round.number === 3" class="mt-2 text-center py-2">
             <span class="text-[10px] font-bold uppercase tracking-widest text-white/20 italic">Eredmények kiértékelése folyamatban...</span>
          </div>
        </div>
        ...
        <div class="pt-4 pb-10">
          <button
            @click="router.push(`/event/${eventId}/summary`)"
...
const results = ref([
  { number: 1, status: 'lezárt', rank: '1', score: '46e', truck: '4e', isExpanded: false },
  { number: 2, status: 'lezárt', rank: '3', score: '27e', truck: '0e', isExpanded: false },
  { number: 3, status: 'lezárt', rank: '-', score: '-', truck: '-', isExpanded: false }
])
...
.border-t-brand-cyan { border-top-color: #00B1E9; }
</style>
