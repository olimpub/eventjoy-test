<template>
  ...
  >
    ...
    <div class="relative h-[120px] w-full group overflow-hidden">
      ...
      <!-- Status Badge & Profile (Single row) -->
      <div class="absolute bottom-2 left-6 right-6 flex items-center justify-between gap-2 z-10">
        <!-- Soft colored Sorsolás status -->
        <span class="px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest shadow-lg flex items-center gap-1.5 transition-colors bg-indigo-400/80 text-white backdrop-blur-md">
    ...
    <div class="px-6 mt-4 relative z-10">
      ...
      <!-- Spinner Section -->
      <div
        @click="router.push(`/event/${eventId}/game`)"
        class="mt-24 flex flex-col items-center justify-center py-12 cursor-pointer group active:scale-95 transition-all"
      >
        <div class="relative w-24 h-24">
          <!-- Animated Spinner -->
          <div class="absolute inset-0 border-4 border-white/5 rounded-full"></div>
          <div class="absolute inset-0 border-4 border-t-brand-cyan border-r-transparent border-b-transparent border-l-transparent rounded-full animate-spin"></div>
          <div class="absolute inset-0 border-4 border-white/10 rounded-full scale-110 blur-sm"></div>
          
          <!-- Inner pulsing circle -->
          <div class="absolute inset-4 rounded-full bg-brand-cyan/20 animate-pulse flex items-center justify-center">
            <div class="w-4 h-4 rounded-full bg-brand-cyan shadow-[0_0_15px_rgba(0,177,233,0.8)]"></div>
          </div>
        </div>
        
        <p class="mt-12 text-white/40 text-sm font-black uppercase tracking-[0.2em] animate-pulse">
